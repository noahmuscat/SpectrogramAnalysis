...
for i = 1:length(conditions)
    ...
    HaraldCombined.(validCondition).ZT_time = [];
    ...
    for j = 1:length(dayDirs)
        ...
        if exist(sleepFile, 'file')
            ...
            HaraldCombined.(validCondition).ZT_time = [HaraldCombined.(validCondition).ZT_time; ZTData.ZT_Hour];
...
mean_day = zeros(size(HaraldCombined.MetaData.fo));
std_day = zeros(size(HaraldCombined.MetaData.fo));
mean_night = zeros(size(HaraldCombined.MetaData.fo));
std_night = zeros(size(HaraldCombined.MetaData.fo));

% Compute mean and std dev per frequency
for freqIdx = 1:length(HaraldCombined.MetaData.fo)
...
%% Save Processed Data
save('HaraldCombinedDataOutliersRemoved.mat', 'HaraldCombined', '-v7.3');
