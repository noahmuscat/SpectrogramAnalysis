...
CanuteV2Combined80 = struct();
CanuteV2Combined80.MetaData = struct();
CanuteV2Combined80.MetaData.Ch = 80; % Channel of interest
CanuteV2Combined80.MetaData.Rat = 'Canute';
CanuteV2Combined80.MetaData.fo = []; % Initialize MetaData.fo as an empty array
...
for i = 1:length(conditions)
    ...
    CanuteV2Combined80.(validCondition) = struct();
    CanuteV2Combined80.(validCondition).ZT_Datetime = [];
    CanuteV2Combined80.(validCondition).SleepState = [];
    CanuteV2Combined80.(validCondition).FractionalPower = {}; % Cell array for fractional power spectra
    ...
    for j = 1:length(dayDirs)
        ...
        if exist(sleepFile, 'file')
            ...
            CanuteV2Combined80.(validCondition).ZT_Datetime = [CanuteV2Combined80.(validCondition).ZT_Datetime; ZTData.ZT_Datetime];
            CanuteV2Combined80.(validCondition).SleepState = [CanuteV2Combined80.(validCondition).SleepState; ZTData.Sleep_State];
        else
        ...
        if exist(eegFile, 'file')
            ...
            if ~isempty(chIdx)
                ...
                if isempty(CanuteV2Combined80.MetaData.fo)
                    CanuteV2Combined80.MetaData.fo = fo; % Save frequencies once
                end
                ...
                for t = 1:min(size(spec, 1), numRecords) % Choose the minimum
                    ...
                    CanuteV2Combined80.(validCondition).FractionalPower{end+1, 1} = fractionalPower;
                end
            else
    ...
    numEntries = length(CanuteV2Combined80.(validCondition).ZT_Datetime);
    if length(CanuteV2Combined80.(validCondition).FractionalPower) > numEntries
        CanuteV2Combined80.(validCondition).FractionalPower = CanuteV2Combined80.(validCondition).FractionalPower(1:numEntries);
    end
end
...
for i = 1:length(conditions)
    ...
    CanuteV2Combined80.(validCondition).CleanedFractionalPower = cell(size(CanuteV2Combined80.(validCondition).FractionalPower));

    % Obtain sleep state and fractional powers
    sleepStates = CanuteV2Combined80.(validCondition).SleepState;
    allFractionalPowers = cat(2, CanuteV2Combined80.(validCondition).FractionalPower{:}); % FxT
    ...
    for s = 1:length(uniqueStates)
        ...
        for idx = stateIndices'
            currentFractionalPower = CanuteV2Combined80.(validCondition).FractionalPower{idx};
            ...
            CanuteV2Combined80.(validCondition).CleanedFractionalPower{idx} = cleanedPower;
        end
    end
end
...
data300Lux = CanuteV2Combined80.(luxField);
...
numFrequencies = length(CanuteV2Combined80.MetaData.fo);
...
for i = 1:length(conditions)
    validCondition = validConditions{i};
    conditionData = CanuteV2Combined80.(validCondition);
    ...
    CanuteV2Combined80.(validCondition).ZScoreFractionalPower = conditionData.ZScoreFractionalPower;
end

%% Save processed data
save('CanuteV2Ch80.mat', 'CanuteV2Combined80', '-v7.3');
